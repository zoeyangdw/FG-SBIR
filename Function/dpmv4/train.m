...
for t = 1:iter
  ...
  for tneg = 1:negiter
    ...
    if isfield(params, 'FAST_RUN') && params.FAST_RUN
        cmd = sprintf('dpmv4/learnfast %.6f %.6f %s %s %s %s %s %s %s %s %s', ...
                  C, J, hdrfile, datfile, modfile, inffile, lobfile, ...
                  cmpfile, objfile, cachedir, logtag);
    else
        cmd = sprintf('dpmv4/learn %.6f %.6f %s %s %s %s %s %s %s %s %s', ...
                  C, J, hdrfile, datfile, modfile, inffile, lobfile, ...
                  cmpfile, objfile, cachedir, logtag);
    end
    fprintf('executing: %s\n', cmd);
...
num = 0;
batchsize = min(pos_batchsize, numpos);
...
scores = [];
complete = 1;
batchsize = min(neg_batchsize, numneg);
